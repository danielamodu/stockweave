...
function decide(input) {
  ...
  const currentBps = currentWeightsBps[sym] || 0;
  const targetBps = targetWeightsBps[sym] || 0;
  // Where the trim lands the asset: selling $notional out of a $navUsd book
  // lowers this asset's weight by notional/nav, never past its target.
  const soldBps = navUsd > 0 ? Math.round((notionalUsd / navUsd) * 10000) : 0;
  const afterBps = Math.max(targetBps, currentBps - soldBps);
  const trade = {
    symbol: sym,
    side: "SELL",
    intoReserve: "USDC",
    notionalUsd,
    fromWeightBps: currentBps,
    targetWeightBps: targetBps,
    afterWeightBps: afterBps,
    overBps,
  };
  const summary =
    `${sym} has grown to ${bps2pct(currentBps)}% of the mix — ${bps2pct(overBps)} points above its ${bps2pct(targetBps)}% target. ` +
    `The agent proposes selling $${notionalUsd} of ${sym} into the USDC reserve, bringing it to about ${bps2pct(afterBps)}%` +
    (capped ? ` (the $${maxPerAction} per-action cap closes part of the gap — a full realignment needs $${neededUsd}).` : ".") +
    " It cannot execute this; only your signature can. Approval required.";
