...
class MirrorWorld {
  ...
  // Phase 7 — fork into independent state. New creator, new PDAs, parent link,
  // copied rule template (version reset to 1). Parent creator gains no
  // authority over the fork (guards use the fork's own creator).
  forkStrategy(parentAddr, newCreator, newStrategyId) {
    const parent = this._strategy(parentAddr);
    if (!newStrategyId || newStrategyId.length > 64) fail("BadStrategyId", "id length");
    const parentRules = this.rules.get(simAddress("rules", parentAddr));
    if (!parentRules) fail("BadRules", "parent has no rules");
    const forkAddr = simAddress("strategy", newCreator, newStrategyId);
    if (this.strategies.has(forkAddr)) fail("BadStrategyId", "already exists");
    this.strategies.set(forkAddr, {
      address: forkAddr, creator: newCreator, strategyId: newStrategyId,
      parentStrategy: parentAddr, status: "ACTIVE", rulesHash: null,
    });
    // Copy the rule template into the fork's OWN rules PDA (version reset to 1).
    const forkRulesAddr = simAddress("rules", forkAddr);
    const { address, strategy, version, ...ruleVals } = parentRules;
    this.rules.set(forkRulesAddr, { address: forkRulesAddr, strategy: forkAddr, ...ruleVals, version: 1 });
    this.strategies.get(forkAddr).rulesHash = forkRulesAddr;
    this.events.push({ event: "StrategyForked", parent: parentAddr, fork: forkAddr, creator: newCreator });
    return forkAddr;
  }
}
